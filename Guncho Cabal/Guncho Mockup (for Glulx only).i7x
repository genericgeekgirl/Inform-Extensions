Version 2/080713 of Guncho Mockup (for Glulx only) by Guncho Cabal begins here.

"This extension simulates Guncho for offline development."
...
To tell (message - indexed text) to (target - an object):
	say "{To [the target]: [message]}[line break]". [mockup]

To tell (message - indexed text) to everyone else near (target - an object):
	say "{To everyone else near [the target]: [message]}[line break]". [mockup]

To tell (message - indexed text) to everyone who can see (spectacle - an object), except the actor:
	say "{To everyone who can see [the spectacle][if except the actor], except the actor[end if]: [message]}[line break]". [mockup]

To announce (message - indexed text):
	say "{To everyone in the realm: [message]}[line break]". [mockup]
...
To send (victim - a PC) to (destination - text):
	say "{Sending [the victim] to '[destination]'}[line break]". [mockup]
...
To decide which number is server register (register name - text):
	decide on 0. [mockup]
To change server register (register name - text) to (new value - number):
	do nothing. [mockup]

To decide which indexed text is server text register (register name - text):
	decide on "". [mockup]
...
To request real-time events every (N - number) seconds:
	say "{Requesting real-time events every [N] seconds}[line break]". [mockup]

To stop real-time events:
	say "{Stopping real-time events}[line break]". [mockup]
...
Carry out triggering real-time events:
	say "{Triggering real-time events}[line break]";
...
This is the investigate multiplayer awareness before action rule:
	abide by the investigate player's awareness before action rule. [mockup]

This is the investigate multiplayer awareness after action and report rule:
	abide by the investigate player's awareness after action rule. [mockup]
