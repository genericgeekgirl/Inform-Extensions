...
Understand "imagination" or "img" or "about" or "credits" or "help" or "hint" or "hints" or "menu" or "walkthrough" as "[meta]".
...
An instructional rule (this is the first we imagine rule):
	if the first we imagine rule is listed in the completed instruction list, make no decision;
		say "[as the parser]Try it out by typing [link]IMAGINE[end link] now.[as normal]";
...
An instructional rule (this is the examine player rule):
	...
	if the first we imagine rule is listed in the completed instruction list:
		say "[as the parser]Now try examining yourself ([link]EXAMINE ME[end link]) to get a better idea of who you are.[as normal]";
...
An instructional rule (this is the teach examining rule):
	...
	if the player can see a non-player thing (called target):
		...
		let N be "[target]";
		say "[as the parser]Individual objects have descriptions, too. Don't forget to [link]EXAMINE[as]EXAMINE [N in upper case][end link] each new item you come across.[as normal]";
...
An instructional rule (this is the teach taking rule):
	...
	if the player can see a portable previously-mentioned non-player thing (called target item):
		...
		say "[as the parser]You can [link]TAKE[as]TAKE [N in upper case][end link] some items.[as normal]";
...
An instructional rule (this is the teach inventory rule): 
	...
	if side-window is not g-present:
		say "[as the parser]To see the list of items you are carrying, type [LINK]INVENTORY[end link].[as normal]";
...
An instructional rule (this is the teach looking rule): 
	...
	if a room is adjacent:
		say "[as the parser]Sometimes getting a new [link]LOOK[end link] at your surroundings can help you figure out what you need to do next. Notice where the exits are and what objects are described.[as normal]";
